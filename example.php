...
if ($hook->hook_exist ( 'filter' )) {
	echo 'Before filtering:</br>' . $urls [0] . '</br>' . $urls [1] . '</br></br>';
	$result = $hook->filter_hook ( 'filter', $urls );
	echo 'After filtering:</br>' . $result [0] . '</br>' . $result [1] . '</br>';
} else {
	echo ('<p><p>no any plugin hooks on filter!!!</p></p>');
}

//print the plugins header
echo "<p>Print all plugins header</p>";
echo "<pre>";
print_r ( $hook->get_plugins_header () );
echo "</pre>";

//print plugins array
echo "<p>Print plugins array</p>";
echo "<pre>";
print_r ( $hook->plugins );
echo "</pre>";
